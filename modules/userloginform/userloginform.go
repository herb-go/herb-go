...
func (m *LoginForm) ID() string {
	return "github.com/herb-go/herb-go/modules/userloginform"
}
...
func (m *LoginForm) Desc(a *app.Application) string {
	return "Create new login form and action"
}
...
func (m *LoginForm) Init(a *app.Application, args *[]string) error {
	...
	m.FlagSet().StringVar(&m.AccountKeyword, "accountkeyword", "", "User account keyword.UID will be used for login if no account keyword given.")
...
func (m *LoginForm) Exec(a *app.Application, args []string) error {
	...
	if !result {
		return errors.New("user system modules not found")
	}
	...
	task := tools.NewTask(filepath.Join(app, "/modules/userloginform/resources"), a.Cwd)
